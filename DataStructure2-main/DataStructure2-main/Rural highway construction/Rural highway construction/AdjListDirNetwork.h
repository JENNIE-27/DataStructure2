...
public:
	...
	int weight;
	KruskalEdge(string v1, string v2, int w);
	KruskalEdge() {};
	KruskalEdge& operator =(const KruskalEdge& Ed);
	bool operator <= (const KruskalEdge & Ed);
	bool operator > (const KruskalEdge & Ed);
	void MiniSpanTreeKruskal(const AdjListDirNetwork& g);
};

KruskalEdge::KruskalEdge(string v1, string v2, int w)
{
	vertex1 = v1;
	vertex2 = v2;
	weight = w;
...
KruskalEdge& KruskalEdge::operator=(const KruskalEdge& Ed)
{
	if (&Ed != this)
	{
		vertex1 = Ed.vertex1;
...
bool KruskalEdge::operator<=(const KruskalEdge& Ed)
{
...
void KruskalEdge::MiniSpanTreeKruskal(const AdjListDirNetwork& g)
{
	int count, VexNum = g.GetVexNum();
	KruskalEdge KEdge;
	MinHeap<KruskalEdge> ha(g.GetArcNum());
	string *kVex, v1, v2;
	kVex = new string[VexNum];
	for (int i = 0; i < VexNum; i++)
		g.GetElem(i, kVex[i]);
	UFSets<string> f(kVex, VexNum);
	...
	while (count < VexNum - 1)
	{
		...
		if (f.Differ(v1, v2)) 
		{
			cout << "边:(" << v1 << "," << v2 << ")权：" << KEdge.weight << endl;
			f.Union(v1, v2);
			count++;
		}
	}
